...
type Step = 'level' | 'duration' | 'pain' | 'session';
const PAIN_LABEL: Record<string, string> = { calentar: 'al calentar', final: 'rango final', carga: 'en carga' };
const zoneName = (id: string) => ZONES.find((z) => z.id === id)?.name ?? id;

const LEVEL_DESC: Record<string, string> = {
  ESCALADO: 'Movimientos adaptados y cargas técnicas. Construyendo la base.',
  INTERMEDIO: 'Técnica dominada, capacidad en construcción.',
  RX: 'Cargas y movimientos prescritos.',
  ELITE: 'Volumen e intensidad de competición.',
  OPEN: 'División individual · cargas oficiales Open.',
  PRO: 'División Pro · cargas oficiales Pro.',
};
...
export function TodaySession({ track, level, levels, onLevelChange }: Props) {
  const t = useTodaySession(track, level ?? levels[0]);
  ...
  if (!t.day)
    return (
      <main className="screen">
        <h1>Descanso</h1>
        <p className="muted">Hoy no hay sesión programada: el descanso también está entrenado.</p>
      </main>
    );
  ...
  if (step === 'level')
    return (
      <main className="screen">
        <p className="eyebrow">{t.day.focus} · paso 1 de 3</p>
        <h1>{track === 'HX' ? '¿En qué división entrenas?' : '¿A qué nivel entrenas hoy?'}</h1>
        <p className="muted">
          {level
            ? 'Confirma tu nivel habitual o cámbialo solo por hoy: cargas y movimientos se ajustan al momento.'
            : 'Elige tu punto de partida. Podrás cambiarlo cualquier día al empezar la sesión.'}
        </p>
        {levels.map((l) => (
          <button
            key={l}
  ...
  if (step === 'duration')
    return (
      <main className="screen">
        <p className="eyebrow">{t.day.focus} · paso 2 de 3</p>
        <h1>¿Cuánto tienes hoy?</h1>
        <p className="muted">
          Bloque de {t.cycle.block.toLowerCase()} (semana {t.cycle.week}/{t.cycle.totalWeeks}). Si eliges 1h,
          mantenemos el estímulo del día y comprimimos lo accesorio.
        </p>
        {(['1h', '2h'] as Duration[]).map((d) => (
          <button key={d} className="option" onClick={() => { t.setDuration(d); setStep('pain'); }}>
            ...
            <span>{d === '1h' ? 'Núcleo del día: lo innegociable.' : 'Sesión completa con fuerza complementaria y extra.'}</span>
          </button>
        ))}
  ...
  if (step === 'pain')
    return (
      <main className="screen">
        <p className="eyebrow">{t.day.focus} · paso 3 de 3</p>
        <h1>¿Alguna molestia hoy?</h1>
        <p className="muted">Adaptamos ejercicios y añadimos movilidad dirigida al final.</p>
        ...
        {!pendingZone ? (
              ...
              {ZONES.map((z) => {
                ...
                return (
                  <button key={z.id} className={`option small ${active ? 'active' : ''}`}
                    onClick={() => active
                      ? t.setPains(t.pains.filter((p) => p.zone !== z.id))
                      : setPendingZone(z.id)}>
          ...
          <>
            <p className="eyebrow">{zoneName(pendingZone)} — ¿cuándo molesta?</p>
            {PAIN_TYPES.map((pt) => (
  ...
  return (
    <main className="screen">
      <p className="eyebrow">Semana {t.cycle.week}/{t.cycle.totalWeeks} · {t.cycle.block} · {level}</p>
      <h1>{t.day.focus}</h1>
      <p className="why">{t.day.why}</p>

      {s.needsActivation.length > 0 && (
        <aside className="notice warn">
          Molestia solo al calentar en {s.needsActivation.map(zoneName).join(' y ')}: añade 6-8 min de
          activación específica antes de empezar. Movilidad dirigida incluida al final.
        </aside>
      )}
          ...
          <section key={b.id} className={`block ${log.done ? 'done' : ''}`}>
            <header>
              ...
              <button className={`check ${log.done ? 'on' : ''}`} aria-label="Completar bloque"
                onClick={() => t.logBlock(b.id, { done: !log.done })}>✓</button>
            </header>
            {b.name && <h2>{b.name}</h2>}
            ...
            <textarea
              className="loginput"
              rows={1}
              placeholder="Comentario · ¿cómo lo has sentido?"
              value={log.comment}
      ...
      {s.omittedCount > 0 && (
        <p className="notice">
          {s.omittedCount} bloque(s) complementario(s) omitido(s) por tiempo. El estímulo clave está intacto.{' '}
          <button className="link" onClick={() => t.setDuration('2h')}>Ver sesión completa</button>
        </p>
      )}
      ...
      {s.mobilityZones.length > 0 && (
        <section className="block mobility">
          ...
          <p className="note">Si la molestia se repite 3 o más días seguidos, coméntalo con un profesional.</p>
        </section>
      )}

      <button className="link" onClick={() => { setStep('level'); t.setDuration(null); t.setPains([]); }}>
        ← Repetir check-in
      </button>
    </main>
  );
